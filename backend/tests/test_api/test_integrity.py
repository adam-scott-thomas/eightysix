...
class TestIntegrity:
    async def test_list_flags_empty(self, client, location):
        """GET integrity-flags with no data returns an empty list."""
        loc_id = str(location.id)
        resp = await client.get(f"/api/v1/locations/{loc_id}/integrity-flags")
        assert resp.status_code == 200
        assert resp.json() == []

    async def test_list_flags_with_data(self, client, db, location):
    ...
    async def test_review_flag_confirmed(self, client, db, location):
    ...
    async def test_review_flag_dismissed(self, client, db, location):
    ...
    async def test_review_flag_invalid_status(self, client, db, location):
    ...
    async def test_review_nonexistent_flag(self, client):
    ...
    async def test_reviewed_flag_not_in_open_list(self, client, db, location):
